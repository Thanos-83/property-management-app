...
import {
  ...
  Zap,
  FileText,
  Lock, // Added Lock icon for the paywall
} from 'lucide-react';
...
} from '@/lib/actions/taskTemplateActions';
...
import { TaskTemplate } from '@/types/taskTemplatesTypes';
...
const SortableItem = ({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`flex gap-2 items-center mb-3 ${isDragging ? 'opacity-70' : ''}`}>
      <FormField
        control={control}
...
export function TaskTemplateSheet({
  ...
  useEffect(() => {
    ...
    if (isCreateMode) {
      ...
      const matchedType = taskTypes?.find(
        (t) => t.id === template.task_type || t.name === template.task_type,
      );

      form.reset({
        name: template.name || '',
        task_type: matchedType ? matchedType.id : template.task_type || '',
        priority: template.default_priority_id
          ? String(template.default_priority_id)
          : '',
        description_notes: template.description_notes || '',
        team_member_id: template.default_team_member_id || 'unassigned',
        offset_minutes: template.linked_properties?.[0]?.offset_minutes || 0,
        checklist: template.checklist_items || [],
        property_ids:
          template.linked_properties?.map((p) => p.property_id) || [],
        is_active:
  ...
  const onSubmit: SubmitHandler<TaskTemplateSchemaType> = async (formData) => {
    const validatedData = {
      ...formData,
      checklist: formData.checklist.map((item, index: number) => ({
        ...item,
        order: index + 1,
      })),
    };
    ...
    try {
      if (isCreateMode) {
        console.log('Create Mode Data: ', validatedData);
        ...
        const response = await updateTaskTemplateAction({
          id: template.id,
          ...validatedData,
        });
        if (!response.success) {
  ...
  const currentTypeObj = taskTypes.find(
    (t) => t.id === currentTypeId || t.name === currentTypeId,
  );
  const isCleaning =
    currentTypeObj?.name === 'Cleaning' || currentTypeId === 'Cleaning';

  return (
    <Sheet open={isOpen} onOpenChange={onOpenChange}>
      <SheetContent className='p-0 flex flex-col w-full sm:max-w-[600px] bg-slate-50 border-l border-border'>
        {/* PAYWALL INTERCEPTOR: Blocks only on creation if over limit */}
        {!canAdd && isCreateMode ? (
          <div className='flex flex-col items-center justify-center h-full p-8 text-center space-y-4'>
            ...
            <SheetDescription className='text-base pb-6'>
              {reason === 'trial_expired'
                ? 'Η δοκιμαστική περίοδος των 14 ημερών έχει λήξει. Αναβάθμισε το πλάνο σου για να δημιουργήσεις περισσότερα templates.'
                : `Έχεις φτάσει το μέγιστο όριο templates για το πλάνο σου (${
                    currentTier === 'basic' ? 'Basic' : 'Trial'
                  }). Αναβάθμισε για να αυτοματοποιήσεις περισσότερες εργασίες.`}
            </SheetDescription>
              ...
              <div className='p-6 pr-14 border-b border-border bg-white shadow-sm shrink-0'>
                <SheetHeader className='text-left space-y-0'>
                  <div className='flex items-center gap-3 mb-2 flex-wrap'>
                    <div
                      className='p-1.5 rounded-md border shadow-sm shrink-0'
                      style={
                        currentTypeObj?.theme_color
                          ? {
                              backgroundColor: `${currentTypeObj.theme_color}1a`,
                              color: currentTypeObj.theme_color,
                              borderColor: `${currentTypeObj.theme_color}33`,
                            }
                          : {}
                      }>
                      {isCleaning ? (
                    ...
                    {!isCreateMode && (
                      <Badge
                        variant={
                          form.watch('is_active') ? 'secondary' : 'outline'
                        }
                        className={`text-[10px] uppercase font-bold tracking-wider shadow-sm ${form.watch('is_active') ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'text-muted-foreground'}`}>
                  ...
                  <SheetDescription className='text-xs font-medium text-muted-foreground'>
                    {isCreateMode
                      ? 'Create a reusable recipe for your tasks.'
                      : 'Modify the automation rules and checklist for this template.'}
                  </SheetDescription>
                </SheetHeader>
              </div>
                      ...
                      <FormItem className='flex flex-row items-center justify-between rounded-lg border border-border p-3 shadow-sm bg-slate-50/50'>
                        <div className='space-y-0.5'>
                          ...
                          <FormDescription className='text-[10px]'>
                            When disabled, this template will be paused and stop
                            generating new automated tasks.
                          </FormDescription>
                        </div>
                        <FormControl>
                          <Switch
                                ...
                                <SelectItem
                                  ...
                                  <div className='flex items-center gap-2'>
                                    <div
                                      ...
                                      style={{
                                        backgroundColor:
                                          priority.priority_color || '#9ca3af',
                                      }}
                                    />
                                    {priority.name}
                                  </div>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription className='text-[10px]'>
                          Tasks created by this template will be automatically
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription className='text-[10px]'>
                          When a booking ends at these properties, this task
