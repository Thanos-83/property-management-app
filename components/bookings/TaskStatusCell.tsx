...
const calculateBadgeProps = (tasks: DetailedTask[]) => {
  if (!tasks || tasks.length === 0) {
    return {
      style:
        'border-dashed border-gray-300 text-muted-foreground hover:border-gray-400 hover:bg-gray-50 hover:text-gray-900',
      content: (
        <>
  ...
  for (const task of tasks) {
    ...
    const isDueSoon = dueDate
      ? dueDate.isSameOrBefore(twoDaysFromNow, 'day')
      : false;
    const isHighPriority = task.priority?.id === 3;
...
export function TaskStatusCell({
  ...
  const { content: badgeContent, style: badgeStyle } =
    calculateBadgeProps(tasks);
                          ...
                          <span className='text-[10px] text-muted-foreground capitalize flex items-center gap-1.5'>
                            ...
                            {task.team_members ? (
                              <span className='text-foreground font-medium '>
                                {task.team_members.first_name}{' '}
                                {task.team_members.last_name?.charAt(0)}.
                              </span>
                            ) : task.team_member_id ? (
                              'Assigned'
                            ) : (
                        ...
                        <div>
                          {task.scheduled_date &&
                            (() => {
                              ...
                              const tomorrow = date.isSame(
                                moment().add(1, 'days'),
                                'day',
                              );
                              const twoDaysFromNow = date.isSame(
                                moment().add(2, 'days'),
                                'day',
                              );
                              return (
                                <span
                                  className={`text-[10px] capitalize flex items-center gap-1.5 ${isToday || tomorrow || twoDaysFromNow ? 'text-red-500 font-bold' : 'text-muted-foreground font-medium'}`}>
                                  {isToday
                                    ? 'Today'
                                    : tomorrow
                                      ? 'Tomorrow'
