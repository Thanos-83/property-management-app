...
import {
  ...
  completeAssignedTaskAction,
  uploadTaskAttachmentRecordAction,
} from '@/lib/actions/teamMemberActions';
...
export function TaskExecutionClient({ initialTask }: TaskExecutionClientProps) {
  ...
  useEffect(() => {
    ...
    getUser();
    setBaseActivities(
      initialTask.task_activity
        .filter((a: TaskActivity) => a.activity_type === 'user_comment')
        .sort(
  ...
  const handleCompleteTask = async () => {
    if (totalCount > 0 && doneCount < totalCount) {
      const confirm = window.confirm(
        "You haven't completed all checklist items. Are you sure you want to finish this task?",
      );
      if (!confirm) return;
    }
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await uploadTaskAttachmentRecordAction(
        initialTask.id,
        publicUrl,
        file.name,
        file.type,
      ...
      <header className='bg-white border-b border-border sticky top-0 z-20 shadow-sm'>
        <div className='flex items-center justify-between p-4'>
          ...
          <div className='flex items-center gap-2'>
            ...
            <Badge
              variant={isTaskDone ? 'secondary' : 'outline'}
              className={`text-[10px] uppercase font-bold tracking-wider ${isTaskDone ? 'bg-status-completed/10 text-status-completed border-status-completed/20' : 'bg-primary/10 text-primary border-primary/20'}`}>
              {initialTask.status}
            </Badge>
          </div>
        </div>
        ...
        <Card className='shadow-sm border-border'>
          <CardContent className='p-4 space-y-3'>
            <div className='flex items-center gap-3 border-b border-border pb-3'>
              <div
                className={`p-2 rounded-lg border shadow-sm ${isCleaning ? 'bg-primary/10 text-primary border-primary/20' : 'bg-chart-2/10 text-chart-2 border-chart-2/20'}`}>
                {isCleaning ? (
              ...
              <div>
                <h2 className='font-bold text-foreground'>
                  {initialTask.property?.title || 'Unknown Property'}
                </h2>
                <p className='text-xs text-muted-foreground flex items-center gap-1 mt-0.5'>
                  <MapPin className='w-3 h-3' />{' '}
                  {initialTask.property?.location || 'No location'}
                </p>
              </div>
            </div>
            ...
            <div className='flex items-center justify-between text-sm'>
              <span className='text-muted-foreground font-medium flex items-center gap-1.5'>
                ...
                {initialTask.scheduled_date
                  ? format(new Date(initialTask.scheduled_date), 'EEEE, MMM do')
                  : 'No Date'}
              </span>
              ...
              <div className='flex flex-col'>
                {optimisticTodos.map((todo: any) => (
                  <div
                    ...
                    <span
                      className={`text-sm leading-snug transition-all ${todo.is_completed ? 'text-muted-foreground line-through' : 'text-foreground font-medium'}`}>
                      {todo.description}
                    </span>
                  </div>
                ))}
            ...
            {initialTask.attachments && initialTask.attachments.length > 0 ? (
              <div className='grid grid-cols-3 gap-2'>
                {initialTask.attachments.map((file: any) => (
                  <div
                    key={file.id}
                    className='relative aspect-square rounded-md overflow-hidden border border-border shadow-sm bg-muted'>
