...
import { Loader2Icon, Building2, MapPin, Link as LinkIcon, Copy, Plus, Trash2, ImageIcon, UploadCloud, Clock, Zap, Save, RefreshCw, CheckCircle2, AlertCircle } from 'lucide-react';
...
import { managePropertySchema, ManagePropertySchemaType } from '@/lib/schemas/property';
import DeleteIcalDialog from './DeleteIcalAlertDialog';
import DeleteIcalDialogAlert from './DeleteIcalAlertDialog';
import { addPropertyIcalAction, deletePropertyAction, deletePropertyIcalAction, togglePropertyTemplateAction, updatePropertyAction, updatePropertyTemplateOffsetAction } from '@/lib/actions/propertiesActions';
...
// ============================================================================
export default function ManagePropertySheet({ isOpen, onOpenChange, property, availableTemplates }: ManagePropertySheetProps) {
  ...
  // --- AUTOMATION SETTINGS STATES ---
   const [localOffsets, setLocalOffsets] = useState<Record<string, HumanReadableOffset>>({});
  ...
  const hasUnsavedChanges = form.formState.isDirty || newCoverPhoto !== null || isCoverPhotoRemoved;
  ...
  const onSubmitDetails = async (data: ManagePropertySchemaType) => {
    ...
    const updatePropertyPayload = {
      ...
      const response = await updatePropertyAction(property.id, updatePropertyPayload);
  ...
  const handleToggleTemplate = async (templateId: string, checked: boolean) => {
    ...
    const existingLinkIndex = optimisticTemplateLinks.findIndex(l => l.template_id === templateId);
    const previousLinks = [...optimisticTemplateLinks];
    let newLinks = [...optimisticTemplateLinks];

    if (existingLinkIndex >= 0) {
      // Update existing link
      newLinks[existingLinkIndex] = { ...newLinks[existingLinkIndex], is_active: checked };
    } else {
  ...
  const handleUpdateOffset = async (templateId: string, newState: HumanReadableOffset) => {
    if (!property) return;
    ...
    setOptimisticTemplateLinks(prev => 
      prev.map(link => link.template_id === templateId ? { ...link, offset_minutes: finalMinutes } : link)
    );

    try {
      // TODO: Call your Server Action to update the `offset_minutes` in the DB
      await updatePropertyTemplateOffsetAction(property.id, templateId, finalMinutes);
  ...
  const handleAddIcal = async (e?: React.MouseEvent | React.KeyboardEvent) => {
    ...
    let hasError = false;
    if (!platform) {
      form.setError('newPlatform', { type: 'manual', message: 'Please select a platform' });
      hasError = true;
    }
    if (!url) {
      form.setError('newIcalUrl', { type: 'manual', message: 'URL is required' });
      hasError = true;
    }
    ...
    try {
        ...
        const response = await addPropertyIcalAction({ propertyId: property.id, platform: platform, icalUrl: url });
  ...
  const handleDeleteIcal = async (icalId: string) => {
    ...
    startTransitionDeleteIcal(async () => {
      ...
      if(!response.error){
        ...
        setOptimisticIcals(prev => prev.filter(ical => ical.id !== icalId));
  ...
  const handleForceSync = async (icalId: string) => {
    ...
    try {
      ...
      setOptimisticIcals(prev => prev.map(ical => {
        // Find the specific sync result for this calendar link
        const specificResult = result.results?.find((r: any) => r.icalSourceId === ical.id);
        ...
        if (specificResult) {
          return {
            ...
            last_synced_at: specificResult.success ? new Date().toISOString() : ical.last_synced_at
          };
        }
        return ical;
      }));
      ...
      if (result.summary.failedSyncs === 0) {
        // 100% Success
        toast.success(`Sync complete! ${result.summary.totalNewBookings} new, ${result.summary.totalUpdatedBookings} updated.`);
      } else if (result.summary.successfulSyncs > 0) {
        // Partial Success (e.g., 3 succeeded, 1 failed)
        toast.warning(`Partial sync: ${result.summary.successfulSyncs} calendars synced, ${result.summary.failedSyncs} failed.`);
      } else {
        // 100% Failure
  ...
  return (
    <Sheet open={isOpen} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-[550px] p-0 flex flex-col h-full bg-slate-50 border-l border-border z-[100]">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmitDetails)} className="flex flex-col h-full overflow-hidden">
            ...
            {/* --- TABS --- */}
            <Tabs defaultValue="details" className="flex-1 gap-0 flex flex-col overflow-hidden">
              <div className="bg-white px-6 pb-1 shrink-0 shadow-sm z-10 border-b border-border">
                <TabsList className="w-full px-1 pb-1 grid grid-cols-3 bg-muted/50">
                  <TabsTrigger value="details" className="text-xs font-bold data-[state=active]:bg-white data-[state=active]:w-full data-[state=active]:shadow-sm">
                    Property Details
                  </TabsTrigger>
                  <TabsTrigger value="sync" className="text-xs font-bold data-[state=active]:bg-white data-[state=active]:w-full data-[state=active]:shadow-sm">
                    Calendar Sync
                  </TabsTrigger>
                  <TabsTrigger value="automations" className="text-xs font-bold data-[state=active]:bg-white data-[state=active]:w-full data-[state=active]:shadow-sm">
                    Automations
                  </TabsTrigger>
                </TabsList>
              </div>
                ...
                {/* TAB 1: DETAILS                             */}
                {/* ========================================== */}
              <TabsContent value="details" className="m-0 outline-none flex-1 flex-col overflow-hidden data-[state=active]:flex">
                      ...
                      <div className="bg-white border border-border rounded-md p-5 shadow-sm space-y-4">
                        ...
                        {previewUrl ? (
                           ...
                           >
                              ...
                              <p className="text-xs text-muted-foreground mt-1">Recommended format: 16:9 ratio (Max 5MB)</p>
                           </div>
                        )}
                      </div>
                        <div className="bg-white border border-border rounded-md p-5 shadow-sm space-y-4">
                        <FormField
                            ...
                            render={({ field }) => (
                            <FormItem>
                                <FormLabel className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Property Title</FormLabel>
                                <FormControl>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            render={({ field }) => (
                                <FormItem>
                                <FormLabel className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Location</FormLabel>
                                <FormControl>
                                    <Input className="bg-muted/20" {...field} />
                                </FormControl>
                                <FormMessage />
                            ...
                            render={({ field }) => (
                                <FormItem>
                                <FormLabel className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Rooms</FormLabel>
                                <FormControl>
                                    <Input type="number" min={1} className="bg-muted/20" {...field} onChange={e => field.onChange(parseInt(e.target.value))} />
                                </FormControl>
                                <FormMessage />
                                </FormItem>
                            )}
                        ...
                        <FormField
                            ...
                            <FormItem>
                                ...
                                <FormControl>
                                <Textarea className="min-h-[100px] bg-muted/20 resize-none" placeholder="Internal notes or description..." {...field} value={field.value || ''} />
                                </FormControl>
                                <FormMessage />
                            </FormItem>
                            )}
                ...
                <div className="p-4 border-t border-border bg-card shrink-0 flex items-center justify-between shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-10 relative">
                  <div className="flex-1">
                    <AlertDialog open={isDeletePropertyOpen} onOpenChange={setIsDeletePropertyOpen}>
                      <AlertDialogTrigger asChild>
                        <Button type="button" variant="ghost" className="text-destructive hover:text-destructive hover:bg-destructive/10 font-semibold px-3">
                      ...
                      <AlertDialogContent className="z-[200]">
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            This will permanently delete this property and all associated calendar links. Tasks and Bookings associated with it will also be deleted. This action cannot be undone.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                          <AlertDialogCancel disabled={isDeletingProperty}>Cancel</AlertDialogCancel>
                          <Button 
                            type="button" 
                            variant="destructive" 
                          ...
                          >
                            {isDeletingProperty ? <Loader2Icon className="w-4 h-4 mr-2 animate-spin" /> : <Trash2 className="w-4 h-4 mr-2" />}
                  ...
                  <div className="flex gap-2 shrink-0">
                    ...
                    <Button 
                      type="submit" 
                      disabled={isDeletingProperty || !hasUnsavedChanges || isSavingDetails} 
                      className="font-bold shadow-sm"
                    >
                      {isSavingDetails ? <Loader2Icon className="w-4 h-4 mr-2 animate-spin" /> : null}
                ...
                {/* TAB 2: CALENDAR SYNC                       */}
                {/* ========================================== */}
                <TabsContent value="sync" className="m-0 outline-none flex-1 flex-col overflow-hidden data-[state=active]:flex">
                  <ScrollArea className="min-h-0 h-full w-full py-4 px-6">
                    <div className="space-y-4">
                        ...
                        <div className="bg-white border border-border rounded-md shadow-sm overflow-hidden">
                            ...
                            <h3 className="text-sm font-bold text-foreground flex items-center gap-2">
                                <LinkIcon className="w-4 h-4 text-muted-foreground" /> Active Connections
                            </h3>
                            </div>
                            ...
                            {optimisticIcals.length === 0 ? (
                                ...
                                optimisticIcals.map(ical =>{
                                   ...
                                   return (
                                      <div key={ical.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-muted/50 transition-colors group border border-transparent hover:border-border">
                                        <div className="flex-1 flex items-center gap-3 overflow-hidden">
                                        <Avatar className={`items-center justify-center bg-white `}>
                                            <AvatarImage src={platformIcons[ical.platform] || '/icons/default.svg'} />
                                            <AvatarFallback>{ical.platform.charAt(0).toUpperCase()}</AvatarFallback>
                                        </Avatar>
                                      ...
                                      <div className="flex items-center gap-2">
                                        ...
                                        {isSyncing ? (
                                          <Badge variant="outline" className="text-[9px] h-4 px-1.5 bg-blue-50 text-blue-700 border-blue-200 gap-1 shadow-sm">
                                            <RefreshCw className="w-2.5 h-2.5 animate-spin mr-[2px]" /> Syncing
                                          </Badge>
                                        ) : ical.sync_status === 'pending' ? (
                                          <Badge variant="outline" className="text-[9px] h-4 px-1.5 bg-muted text-muted-foreground border-border gap-1 shadow-sm">
                                            <Clock className="w-2.5 h-2.5 mr-[2px]" /> Pending
                                          </Badge>
                                        ) : ical.sync_status === 'error' ? (
                                          <Badge variant="destructive" className="text-[9px] h-4 px-1.5 gap-1 shadow-sm">
                                            <AlertCircle className="w-2.5 h-2.5 mr-[2px]" /> Failed
                                          </Badge>
                                        ) : (
                                          <Badge variant="secondary" className="text-[9px] h-4 px-1.5 gap-1 bg-emerald-100 text-emerald-700 hover:bg-emerald-100 border-emerald-200 shadow-sm">
                                            <CheckCircle2 className="w-2.5 h-2.5 mr-[2px]" /> Healthy
                                          </Badge>
                                        )}
                                      </div>
                                      
                                      <span className="text-xs text-muted-foreground truncate mt-0.5" title={ical.ical_url}>{ical.ical_url}</span>
                                      
                                      {/* TIMESTAMP */}
                                      <span className="text-[10px] text-muted-foreground mt-1 flex items-center gap-1">
                                        <Clock className="w-3 h-3" /> {getRelativeTime(ical.last_synced_at)}
                                      </span>
                                    </div>
                                    ...
                                    <Button type="button" variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-foreground" onClick={() => copyToClipboard(ical.ical_url)}>
                        ...
                        <div className="bg-white border border-border rounded-md shadow-sm overflow-hidden">
                            <div className="px-5 py-3 border-b border-border bg-muted/30">
                            <h3 className="text-sm font-bold text-foreground flex items-center gap-2">
                                <Plus className="w-4 h-4 text-muted-foreground" /> Add New Link
                            </h3>
                            </div>
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                <FormItem>
                                    <FormLabel className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Platform</FormLabel>
                                    ...
                                    <SelectContent className="z-[200]">
                                        ...
                                        <SelectItem key={option.value} value={option.value}>
                                            <div className="flex items-center gap-2">
                                            <Image src={option.icon} alt={option.label} width={16} height={16} className="object-contain" />
                                            {option.label}
                                            </div>
                                        </SelectItem>
                                        ))}
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                <FormItem>
                                    <FormLabel className="text-xs font-bold text-muted-foreground uppercase tracking-wider">iCal URL</FormLabel>
                                    <FormControl>
                                    <Input 
                            ...
                            >
                                {isAddingIcal ? <Loader2Icon className="w-4 h-4 mr-2 animate-spin" /> : null}
                ...
                {/* TAB 3: AUTOMATIONS                         */}
                {/* ========================================== */}
                <TabsContent value="automations" className="m-0 outline-none flex-1 flex-col min-h-0 data-[state=active]:flex">
                      ...
                      <div className="bg-white border border-border rounded-md shadow-sm overflow-hidden">
                        <div className="px-5 py-4 border-b border-border bg-gradient-to-r from-muted/50 to-muted/10">
                          <h3 className="text-sm font-bold text-foreground flex items-center gap-2 mb-1">
                            <Zap className="w-4 h-4 text-primary" /> Task Automations
                          </h3>
                          <p className="text-xs text-muted-foreground leading-relaxed">
                            Link task templates to this property. When an iCal booking syncs, active templates will automatically generate tasks for your team.
                          </p>
                        </div>
                        ...
                        <div className="p-4 space-y-4 bg-slate-50/50">
                          {availableTemplates.map((template) => {
                            ...
                            const currentLink = optimisticTemplateLinks.find(link => link.template_id === template.id);
                            ...
                            const uiState = localOffsets[template.id] || currentParsedState;
                            // Check if local state differs from db state
                            const hasDirtyOffset = localOffsets[template.id] !== undefined && calculateMinutes(localOffsets[template.id]) !== dbOffset;

                            return (
                               <div key={template.id} className={`bg-white border rounded-xl overflow-hidden transition-all duration-200 ${isActive ? 'border-primary/40 shadow-sm' : 'border-border shadow-sm opacity-80'}`}>
                                    ...
                                    <div className="flex items-center justify-between gap-4">
                                      <div>
                                        <label className="text-xs font-bold text-foreground flex items-center gap-1.5">
                                          <Clock className="w-3.5 h-3.5" /> Scheduling Rule
                                        </label>
                                      ...
                                      <Button 
                                        type="button" 
                                        size="sm"
                                        variant={hasDirtyOffset ? "default" : "secondary"}
                                        disabled={!hasDirtyOffset || isUpdatingOffset === template.id}
                                        onClick={() => handleUpdateOffset(template.id, uiState)}
                                        className="h-8 px-3 shadow-sm transition-all shrink-0"
                                      >
                                        {isUpdatingOffset === template.id || isPendingUpdateOffset  ? (
                                          <Loader2Icon className="w-3.5 h-3.5 animate-spin" />
                                        ) : (
                                          <Save className="w-3.5 h-3.5 mr-1.5" />
                                        )}
                                        {isUpdatingOffset === template.id ? "" : "Save"}
                                      </Button>
                                    </div>
                                    ...
                                    <div className="flex items-center flex-wrap gap-2">
                                      ...
                                      <div className="relative w-20 shrink-0">
                                        <Input 
                                          ...
                                          onChange={(e) => {
                                            ...
                                            setLocalOffsets(prev => ({...prev, [template.id]: { ...uiState, value: val }}));
                                          }}
                                      ...
                                      <Select 
                                        ...
                                        onValueChange={(val: TimeUnit) => 
                                          setLocalOffsets(prev => ({...prev, [template.id]: { ...uiState, unit: val }}))
                                        }
                                      >
                                      ...
                                      <Select 
                                        ...
                                        onValueChange={(val: TimeDirection) => 
                                          setLocalOffsets(prev => ({...prev, [template.id]: { ...uiState, direction: val }}))
                                        }
                                      >
                                        ...
                                        <SelectContent className="z-[200]">
                                          <SelectItem value="after">After Checkout</SelectItem>
                                          <SelectItem value="before">Before Check-in</SelectItem>
                                        </SelectContent>
                                      </Select>
                          ...
                          {availableTemplates.length === 0 && (
                            <div className="text-center p-6 border-2 border-dashed border-border rounded-xl">
                              ...
                              <p className="text-xs text-muted-foreground">Go to your <Link href="/dashboard/task-templates/new-template" className="text-primary font-bold underline underline-offset-2 decoration-dashed">Task Templates</Link> to create rules.</p>
                            </div>
                          )}
