...
} from '@/lib/actions/taskMemberActions';
...
export default function TaskMembersClient({
  initialMembers,
  taskTypes,
}: TaskMembersClientProps) {
        ...
        {initialMembers.length === 0 ? (
          <div className='flex flex-col items-center justify-center p-12 border-2 border-dashed border-border rounded-xl bg-white/50 text-center mt-8'>
            ...
            <p className='text-sm text-muted-foreground mb-4'>
              Invite your first cleaner or maintenance staff to get started.
            </p>
          </div>
        ) : (
          <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4'>
            {initialMembers.map((member) => {
              ...
              if (isPending && member.invite_details) {
                isExpired =
                  new Date(member.invite_details.expires_at) < new Date();
                isMaxedOut =
                  member.invite_details.click_count >=
                  member.invite_details.max_clicks;
              }
              ...
              if (isPending) {
                if (isExpired) {
                  badgeContent = (
                    <>
                      <AlertCircle className='w-3 h-3' /> Expired Link
                    </>
                  );
                  badgeStyles =
                  ...
                  badgeContent = (
                    <>
                      <AlertCircle className='w-3 h-3' /> Max Clicks Reached
                    </>
                  );
                  badgeStyles =
                    'bg-destructive/10 text-destructive border-destructive/20';
              ...
              return (
                <Card
                  key={member.id}
                  className={`py-0 rounded-md overflow-hidden transition-all ${isPending ? 'border-dashed bg-slate-50/50' : 'bg-white shadow-sm hover:shadow-md'} ${isDeleting ? 'opacity-50 animation-pulse' : ''}`}>
                    ...
                    <div className='p-5 flex flex-col gap-4'>
                      ...
                      <div className='flex justify-between items-start'>
                        <div className='flex items-center gap-3'>
                          <Avatar
                            className={`h-10 w-10 border ${isPending ? 'opacity-60 grayscale' : ''}`}>
                          ...
                          <div className='flex flex-col'>
                            <span
                              className='font-bold text-sm text-foreground truncate max-w-[140px]'
                              title={fullName}>
                              {fullName}
                            </span>
                      ...
                      <div className='pt-4 border-t border-border flex items-center justify-between'>
                        ...
                        <Badge
                          ...
                          className={`shadow-sm gap-1 text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 ${badgeStyles}`}>
                          {badgeContent}
                        </Badge>
