                ...
                <div className='flex flex-col gap-3'>
                  {bookings.map((event) => {
                    ...
                    const guestName =
                      ...
                      event.resource?.originalData?.guest_name ||
                      'Unknown Guest';
                    const propertyName =
                      event.resource?.propertyName ||
                      event.resource?.originalData?.properties?.title ||
                      'Unknown Property';
                    const status =
                      event.resource?.originalData?.status || 'Confirmed';
                    ...
                    return (
                      <div
                        ...
                        <div className='flex items-start gap-4 pl-2 min-w-0 w-full'>
                          ...
                          <div className='flex flex-col justify-center min-w-0 flex-1 pr-4'>
                            ...
                            <div className='flex items-center gap-2 mb-1'>
                              ...
                              <Badge
                                className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border ${getStatusStyles(status)}`}>
                                {status}
                              </Badge>
                            </div>
                        ...
                        <div className='shrink-0 pl-2 self-center'>
                          <Button
                            variant={
                              hasMultipleBookings ? 'destructive' : 'secondary'
                            }
                            size='sm'
              ...
              <div className='flex flex-col gap-4 mt-4'>
                ...
                <div className='flex flex-col gap-3'>
                  {tasks.map((event) => {
                    ...
                    const propertyName =
                      event.resource?.propertyName ||
                      event.resource?.originalData?.property?.title ||
                      'Unknown Property';
                    const assignee = event.resource?.originalData?.team_members
                      ? event.resource?.originalData?.team_members?.first_name +
                        ' ' +
                        event.resource?.originalData?.team_members?.last_name
                      : 'Unassigned';
