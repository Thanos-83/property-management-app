...
export function TaskDetailsSheet({
  ...
  useEffect(() => {
    ...
    if (isCreateMode) {
      ...
      form.reset({
        ...
        status: matchStatus
          ? String(matchStatus.status).trim()
          : (task?.status ?? ''),
  ...
  const onSubmit: SubmitHandler<
    TaskDetailsSchemaType & { type?: string }
  > = async (data) => {
    ...
    const payload = {
      ...
      team_member_id:
        data.team_member_id === 'unassigned' ? null : data.team_member_id,
    ...
    if (!result?.error) {
      ...
      toast.error(
        isCreateMode ? 'Failed to create task' : 'Failed to update task',
      );
    }
  };
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true); // 1. Start spinner in the modal

    try {
      const result = await deleteTaskByIdAction(task.id); // 2. Wait for backend

      if (!result.error) {
        // 3. SUCCESS UX: Close UI immediately so it feels fast
        ...
        // 5. Let the table refresh in the background
        router.refresh();
      } else {
  ...
  const currentStatusObj = taskStatus?.find(
    (s: TaskStatusOption) =>
      s.status?.toLowerCase().trim() ===
      form.watch('status')?.toLowerCase().trim(),
  );
        ...
        <Form {...form}>
          <form
            ...
            <Tabs
              ...
              {/* --- STICKY HEADER --- */}
              <div
                className={`p-4 pb-2 relative z-10 ${isCreateMode ? 'shadow-sm' : ''}`}>
                <SheetHeader className='text-left space-y-0 pr-8'>
                  <div className='flex items-center gap-3 mb-1.5 flex-wrap'>
                    <div
                      className={`p-1.5 rounded-md border shadow-sm ${isCleaning ? 'bg-primary/10 text-primary border-primary/20' : 'bg-chart-2/10 text-chart-2 border-chart-2/20'}`}>
                      {isCleaning ? (
                    ...
                    <Badge
                      ...
                      style={
                        !isCreateMode && currentStatusColor
                          ? {
              ...
              {!isCreateMode && (
                <div className='px-6 bg-card shrink-0 shadow-sm z-10'>
                  <TabsList className='w-full grid grid-cols-2 bg-muted/50 h-8 mb-2'>
                    <TabsTrigger
                      value='details'
                      className='text-xs font-bold data-[state=active]:bg-background data-[state=active]:shadow-sm'>
                      Task Details
                    </TabsTrigger>
                    <TabsTrigger
                      value='history'
              ...
              <ScrollArea className='flex-1 px-6 py-1 space-y-6 overflow-hidden'>
                ...
                <TabsContent className='space-y-6 py-1' value='details'>
                  ...
                  {!isCreateMode &&
                    form.watch('team_member_id') === 'unassigned' &&
                    !['completed', 'cancelled'].includes(
                      currentStatusLabel?.toLowerCase()?.trim() || '',
                    ) && (
                      <div className='bg-warning/10 border border-warning/20 rounded-md p-4 flex items-start gap-3 shadow-sm'>
                        ...
                        <div>
                          ...
                          <p className='text-xs text-warning/80 mt-1 font-medium leading-relaxed'>
                            This task currently has no team member assigned to
                            it. Please assign someone to ensure it gets
                            completed.
                          </p>
                        </div>
                      </div>
                              ...
                              <PopoverTrigger asChild>
                                <FormControl>
                                  <Button
                                    variant='outline'
                                    className={cn(
                                      'w-full justify-start text-left font-normal border-border bg-white',
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <Select
                              ...
                              <FormControl>
                                <SelectTrigger
                                  className={`w-full bg-white border-border ${field.value === 'unassigned' ? 'text-warning font-semibold' : ''}`}>
                              ...
                              <Select
                                ...
                                <FormControl>
                                  <SelectTrigger
                                    ...
                                    style={
                                      selectedStatusObj?.status_color
                                        ? {
                                            color:
                                              selectedStatusObj.status_color,
                                          }
                              ...
                              <Select
                                ...
                                <FormControl>
                                  <SelectTrigger
                                    className='w-full bg-white border-border font-medium'
                                    style={
                                      selectedPriorityObj?.priority_color
                                        ? {
                                            color:
                                              selectedPriorityObj.priority_color,
                                          }
                  ...
                  <div className='bg-white border border-border rounded-sm p-5 shadow-sm space-y-4'>
                    <div className='flex items-center justify-between mb-4'>
                      ...
                      <span
                        className={`text-xs font-bold px-2 py-0.5 rounded-full ${completedCount === fields.length && completedCount > 0 ? 'bg-status-completed/10 text-status-completed' : 'bg-muted text-muted-foreground'}`}>
              ...
              <div className='p-4 border-t border-border bg-card shrink-0 flex items-center justify-between shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]'>
                ...
                <div className='flex gap-2 shrink-0'>
                  ...
                  <Button
                    type='submit'
                    disabled={
                      isDeleting || !hasChanges || form.formState.isSubmitting
                    }
