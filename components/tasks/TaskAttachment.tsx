        ...
        <div className='grid grid-cols-2 sm:grid-cols-3 gap-3 mt-4'>
            ...
            <div
              ...
              {attachment.file_type?.startsWith('image/') ? (
                ...
                <Link
                  ...
                  target='_blank'
                  rel='noopener noreferrer'
                  className='flex flex-col items-center justify-center h-full text-muted-foreground p-2 text-center bg-card'>
                  <FileText className='w-8 h-8 mb-2 opacity-50 text-primary' />
                  <span
                    className='text-[10px] truncate w-full px-1'
                    title={attachment.file_name}>
          ...
          {newFiles.map((file, index) => {
            ...
            return (
              <div
                key={`new-${index}`}
                className='relative group rounded-md overflow-hidden border border-primary/30 ring-1 ring-primary/20 bg-primary/5 aspect-square'>
                {isImage && objectUrl ? (
                  <Image
                    src={objectUrl}
                    alt={file.name}
