            ...
            {sortedActivities.length === 0 ? (
              ...
              sortedActivities.map((act: any) => {
                ...
                const matchedMember = teamMembers.find(
                  (tm: any) => tm.auth_member_id === act.user_id,
                );
                ...
                if (isSystem) {
                  return (
                    <div
                      key={act.id}
                      className='flex items-start gap-3 text-xs text-muted-foreground bg-white p-3 rounded-lg border border-border/50 shadow-sm'>
                      ...
                      <div className='flex flex-col flex-1 gap-1 sm:flex-row sm:items-center sm:gap-2'>
                        ...
                        <span className='flex-1 text-foreground/80'>
                          {renderLogContent(act.content)}
                        </span>
                        <span className='text-[10px] opacity-60 font-medium sm:ml-auto'>
                          {timeString}
                        </span>
                      </div>
                    </div>
                ...
                return (
                  <div
                    ...
                    <Avatar
                      className={`w-8 h-8 rounded-full border shadow-sm mt-1 ${isMe ? 'border-primary/20' : 'border-border'}`}>
                      <AvatarImage
                        src={avatarUrl || ''}
                        className='object-cover'
                      />
                      <AvatarFallback
                        className={`text-[10px] font-bold ${isMe ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>
                    ...
                    {/* Bubble Container */}
                    <div
                      className={`flex flex-col flex-1 ${isMe ? 'items-end' : 'items-start'}`}>
